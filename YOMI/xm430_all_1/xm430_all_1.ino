...
// 모터별 최소 및 최대 각도 설정
int min_angle[10] = {0, 0, 0, 0, 
                     80, 100, 90, 180, 
                     0, 0};  // 모터별 최소 각도
int max_angle[10] = {360, 360, 360, 360, 
                     240, 260, 270, 265, 
                     360, 360};  // 모터별 최대 각도

void set_all_motors(Stream &serialport, float velocity[10], float position[10]) {
  float position_limited[10];
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (position[i] < min_angle[i]) {
      position_limited[i] = min_angle[i];
    } else if (position[i] > max_angle[i]) {
      position_limited[i] = max_angle[i];
    } else {
      position_limited[i] = position[i];
    }
