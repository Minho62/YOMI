...
from tkinter import filedialog, simpledialog  # 파일 선택 대화상자
...
class MotorControlGUI:
    def __init__(self, root):
        ...
        rospy.init_node('yomi_gui', anonymous=True)
    ...
    def set_slider_positions(self):
        ...
        self.position_slider_ranges = [
            (120, 300),(110, 260),(90, 270),(95, 180),
            (60, 240),(100, 250),(90, 270),(180, 265),   # Motor 10
            (120, 240),(110, 250)
        ]
        self.servo_slider_ranges = [
            (0, 90), (0, 130), (0, 140), (0, 125),
            (0, 90), (0, 110), (0, 120), (0, 145)
        ]
        # ─────────────────────────────────────────────────────────────────
    ...
    def create_widgets(self):
        ...
        for i in range(self.servo_count):
            ...
            angle_range = self.servo_slider_ranges[i]
            angle_slider = tk.Scale(
                self.root,
                from_=angle_range[0], to=angle_range[1],
        ...
        # ───────────────────────────────────────────────
        # "Save Motion" 버튼 추가 - 모션 데이터를 파일로 저장
        ...
        # "Load Motion" 버튼 추가 - 파일에서 모션 데이터를 읽어 슬라이더 업데이트
        ...
        self.play_sequence_button = tk.Button(
            self.root,
            text="▶ Play Sequence",
            command=self.play_sequence_from_folder,
            font=("Arial", 22, "bold"),
            width=20,
            height=1,
            bd=5,
        ...
        self.canvas.create_window(300, 900, window=self.play_sequence_button)
    ...
    # ───────────────────────────────────────────────
    # 스위치 버튼 및 토픽 처리 관련 함수
    def create_switch_widgets(self):
        """4개의 스위치 버튼 생성 (추가)"""
        self.switch_buttons = []
        positions = [(1200, 50), (1300, 50),(1400, 50), (1500, 50), (1600, 50), (1700, 50)]
        labels = ["Sw 1", "Sw 2", "Sw 3", "Sw 4", "Sw 5", "Sw 6"]
        for i in range(6):
            btn = tk.Button(
                self.root,
                text=labels[i],
    ...
    def save_motion(self):
        ...
        # 사용자에게 타임스탬프(ms) 입력 받기
        timestamp_ms = simpledialog.askinteger(
            "Timestamp 입력", 
            "이 동작의 실행 시점 (ms)을 입력하세요:", 
            minvalue=0, 
            initialvalue=1000
        )
        if timestamp_ms is None:
            print("⛔ Timestamp 입력 취소됨")
            return

        motion_data = motor_speeds + motor_positions + servo_angles + [timestamp_ms]
        #filename = "/home/micca/catkin_ws/src/yomi/motion/motion_" + time.strftime("%Y%m%d_%H%M%S") + ".csv"
        # 사용자에게 저장 경로 및 파일명 선택 받기
    ...
    # 모션 데이터를 파일에서 읽어 슬라이더 업데이트하는 함수
    def load_motion_file(self):
        ...
        try:
            ...
            values = [int(x) for x in data_str.split(",")[0:28]]  # timestamp 제외하고 슬라이더만 읽기
    ...
    def play_sequence_from_folder(self):
        import os, glob
        folder = filedialog.askdirectory(
            title="Select Motion Sequence Folder",
            initialdir="/home/micca/catkin_ws/src/yomi/motion"
        )
        if not folder:
        ...
        file_list = sorted(glob.glob(os.path.join(folder, "*.txt")))
        if not file_list:
            print("No .txt files found in selected folder.")
            return

        threading.Thread(target=self._play_sequence_thread, args=(file_list,), daemon=True).start()

    def _play_sequence_thread(self, file_list):
        import time, os
        print(f"▶ Playing sequence of {len(file_list)} steps")
        ...
        for filepath in file_list:
            try:
                with open(filepath, "r") as f:
                    ...
                    if len(values) != 29:
                        raise ValueError("Expected 29 values: 28 motion values + 1 timestamp")
                ...
                self.apply_motor_data()
                self.apply_servo_data()
                print(f"  ✅ Executed {os.path.basename(filepath)}")

            except Exception as e:
                print(f"⚠️ Failed to execute {filepath}: {e}")
